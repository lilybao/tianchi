    ...
    public static Map<String, Integer> toAllMaxInstancePerNodeLimit(Rule rule, List<GroupRuleAssociate> groupRuleAssociates) {
        ...
        for (ReplicasMaxInstancePerNode replicasMaxInstancePerNode : rule.getReplicasMaxInstancePerNodes()) {
            ...
            if (restrain.equals(Restrain.GE)) {//此规则用意为应用实例数大于或等于(GE)replicas的应用，最大堆叠数量为maxInstancePerNode。
                groupRuleAssociates.parallelStream().filter(app -> app.getReplicas() >= replicas)
                    .forEach(app -> allMaxInstancePerNodeLimit.put(app.getGroup(), maxInstancePerNode));
                continue;
            }

            if (restrain.equals(Restrain.LE)) {
                groupRuleAssociates.parallelStream().filter(app -> app.getReplicas() <= replicas)//若存在冲突以最小的为准，实际不会出现这种情况。
                    .forEach(app -> allMaxInstancePerNodeLimit.compute(app.getGroup(),
        ...
        groupRuleAssociates.parallelStream().filter(app -> !existsRuleGroups.contains(app.getGroup()))//未存在规则的应用分组添加默认的堆叠上限。
            .forEach(app -> allMaxInstancePerNodeLimit.put(app.getGroup(), rule.getDefaultMaxInstancePerNode()));
