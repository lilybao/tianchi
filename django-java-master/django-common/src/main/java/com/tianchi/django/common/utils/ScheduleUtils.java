...
public class ScheduleUtils {
    ...
    public static boolean staticFillOnePod(NodeWithPod nwp, Pod pod, Map<String, Integer> allMaxInstancePerNodeLimit) {
        ...
        if (!ScheduleUtils.resourceFillOnePod(nwp, pod)) {
            return false;
        }

        if (!ScheduleUtils.layoutFillOnePod(allMaxInstancePerNodeLimit, nwp, pod)) {
            return false;
        }

        PodPreAlloc podPreAlloc = ScheduleUtils.cgroupFillOnePod(nwp, pod);
