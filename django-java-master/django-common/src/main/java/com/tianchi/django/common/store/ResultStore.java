...
public class ResultStore {
    ...
    public static void storeSchedule(@NonNull List<ScheduleResult> list, @NonNull String directory) throws IOException {

        String result = JsonTools.toSimplifyJson(list);

        File file = Paths.get(RESULT_DIRECTORY, directory, SCHEDULE_RESULT).toFile();

        log.info("schedule file : {}", file.getAbsolutePath());

        FileUtils.writeStringToFile(file, result, Charsets.UTF_8);

    }

    public static void storeReschedule(@NonNull List<RescheduleResult> list, @NonNull String directory) throws IOException {

        String result = JsonTools.toSimplifyJson(list);

        File file = Paths.get(RESULT_DIRECTORY, directory, RESCHEDULE_RESULT).toFile();

        log.info("reschedule file : {}", file.getAbsolutePath());
