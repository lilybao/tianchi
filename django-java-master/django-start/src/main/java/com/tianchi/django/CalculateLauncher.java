...
public class CalculateLauncher {
    ...
    private static void execute(long start, @NonNull List<String> directorys) throws InterruptedException {
        ...
        for (String directory : directorys) {
            ...
            maxTimeLimitInMins = Math.max(0, rule.getTimeLimitInMins());
    ...
    private static void schedule(String directory, long start, Rule rule, List<Node> nodes, List<App> apps) throws Exception {
        ...
        //执行静态布局功能
        List<ScheduleResult> result = iSchedule.schedule(
            nodes.parallelStream().map(Node::copy).collect(toList()), apps.parallelStream().map(App::copy).collect(toList()), rule.copy()
        );

        ResultStore.storeSchedule(result, directory);

        List<NodeWithPod> nodeWithPods = NodeWithPodUtils.resultToNodeWithPods(nodes, apps, result);
    ...
    private static void reschedule(String directory, long start, Rule rule, List<NodeWithPod> nodeWithPods) throws Exception {
        ...
        //执行动态迁移功能
        List<RescheduleResult> result = iReschedule.reschedule(
            nodeWithPods.parallelStream().map(NodeWithPod::copy).collect(toList()), rule.copy()
        );

        ResultStore.storeReschedule(result, directory);
